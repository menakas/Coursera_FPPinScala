...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    throw java.util.NoSuchElementException("Empty TweetSet")
  /**
   * The following methods are already implemented
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    if this.isInstanceOf[Empty] then
      acc
    else if p(this.elem) then 
      this.right.filterAcc(p,this.left.filterAcc(p,acc.incl(this.elem)))
    else 
      this.right.filterAcc(p,this.left.filterAcc(p,acc))

  def union(that: TweetSet): TweetSet = //unionAcc(that.filter(tweet => !this.contains(tweet) ),Empty())  
    var res = this.asInstanceOf[TweetSet]
    this.foreach( tweet => res = res.incl(tweet))
    that.foreach(tweet =>
      if !res.contains(tweet) then
        res = res.incl(tweet)
    )
    res 
  ...
  def mostRetweeted: Tweet =
    var mx = -1
    var maxtweet = null.asInstanceOf[Tweet]
    this.foreach(tweet => 
      if tweet.retweets > mx then
        mx = tweet.retweets
        maxtweet = tweet
      )
    maxtweet
  ...
  def descendingByRetweet: TweetList =
    val elem = this.mostRetweeted
    Cons(elem, this.remove(elem).descendingByRetweet)
